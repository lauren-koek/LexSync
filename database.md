# Database

PostgreSQL 16. SQLAlchemy 2.x models in `db/models.py`. Tables are created automatically on first pipeline run via `create_tables()`.

## Current schema — `documents` table

| Column | Type | Nullable | Description |
|---|---|---|---|
| `id` | UUID | No | Primary key, auto-generated |
| `source_url` | text | No | Document page URL (unique) |
| `doc_type` | varchar(100) | Yes | e.g. `"Circular"`, `"Guidelines"` |
| `date` | date | Yes | Publication date parsed from scraper output |
| `title` | text | Yes | Document title |
| `topic` | text | Yes | Regulatory topic from MAS |
| `tags` | jsonb | Yes | Array of free-text tags |
| `applies_to` | jsonb | Yes | Array of entity types the document applies to |
| `related_items` | jsonb | Yes | Array of related document URLs |
| `pdf_url` | text | Yes | Direct PDF download URL |
| `pdf_local_path` | text | Yes | Local path of downloaded PDF (reserved, not yet populated) |
| `ocr_text` | text | Yes | Full extracted text from PDF |
| `llm_summary` | text | Yes | 80–150 word LLM-generated summary |
| `llm_categories` | jsonb | Yes | Array of category strings from the standard tag list |
| `llm_impact_check` | text | Yes | LLM-generated impact check block |
| `llm_summary_flagged` | boolean | Yes | Set by `backend/llm/hallucination_check.py` — true if `llm_summary` contains a number/date not found in the source text, or drifted vocabulary. Advisory only. |
| `llm_summary_check_notes` | text | Yes | Human-readable explanation from the same check (e.g. which numbers were unsupported) |
| `scraped_at` | timestamptz | Yes | Timestamp of last scrape |
| `processed_at` | timestamptz | Yes | Timestamp of last LLM processing |
| `created_at` | timestamptz | No | Row creation time (server default) |
| `updated_at` | timestamptz | Yes | Row last-update time (auto on update) |

`source_url` carries a unique constraint — pipeline runs upsert on it.

## Internal-document index (pgvector) — `internal_document_chunks` table

The persistence layer is split into two responsibilities:

1. The regulatory-ingestion table above (`documents`), for downloaded PDF
   text, source metadata, and LLM recommendations. No vector column.
2. `internal_document_chunks` (`backend/db/models.py::InternalDocumentChunk`),
   which owns the pgvector embedding column and its vector index. This is
   the persistent, database-backed counterpart to the request-scoped,
   in-memory Qdrant index in `backend/analysis/store.py` — chunks written
   here survive process restarts and are indexed once instead of being
   re-embedded on every analysis request.

| Column | Type | Nullable | Description |
|---|---|---|---|
| `id` | UUID | No | Primary key, auto-generated |
| `doc_id` | text | No | Groups chunks back to their source document (e.g. filename stem) |
| `title` | text | No | `"{doc_id} — {clause_reference}"`, from `ingest.py` |
| `clause_reference` | text | No | Citation, e.g. `"Clause 8"`, `"Section 12A (part 2)"` |
| `content` | text | No | Chunk text |
| `embedding` | vector(384) | No | pgvector embedding, see below |
| `created_at` | timestamptz | No | Row creation time (server default) |

An HNSW index (`vector_cosine_ops`) is created on `embedding` — see
`backend/analysis/internal_index.py` for the read/write API
(`build_index()`, `find_impacted_assets()`) and its embedding strategy
(hosted embedding model when `OPENROUTER_API_KEY` is set, deterministic
offline hashing fallback otherwise). `create_tables()` runs
`CREATE EXTENSION IF NOT EXISTS vector` before creating tables, since the
`vector` column type requires the extension to already exist.

Regulatory PDFs and LLM recommendations do not receive vector columns.
FastEmbed, Qdrant, and ONNX Runtime remain confined to `store.py`'s
in-memory, request-scoped path and are not used by this table.

## Running locally with Docker

`docker-compose.yml` uses `pgvector/pgvector:pg16` (not plain `postgres:16`)
so `CREATE EXTENSION vector` succeeds — it's the same Postgres 16 with the
pgvector extension binaries preinstalled.

Start only Postgres:

```bash
docker-compose up postgres -d
```

Then run the pipeline (tables are created automatically):

```bash
DATABASE_URL=postgresql://lexsync:lexsync@localhost:5432/lexsync python -m backend.pipeline
```

Or put `DATABASE_URL` in `.env` and omit the prefix.

## Connecting to the DB

```bash
psql postgresql://lexsync:lexsync@localhost:5432/lexsync
```

`DATABASE_URL` format:

```
postgresql://<user>:<password>@<host>:<port>/<dbname>
```

Default credentials from `docker-compose.yml`: user `lexsync`, password `lexsync`, db `lexsync`.

## Resetting / recreating tables

Drop and recreate from a psql session:

```sql
DROP TABLE documents;
```

Then re-run the pipeline — `create_tables()` will recreate the schema.

To reset via Python:

```python
from backend.db.models import Base
from backend.db.session import engine

Base.metadata.drop_all(engine)
Base.metadata.create_all(engine)
```
